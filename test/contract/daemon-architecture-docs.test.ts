...
describe("v2 daemon architecture records", () => {
  test("supersedes the Actions scheduler and dedicated macOS user", async () => {
    const [decision, actions, isolation, scheduler, oldDesign] = await Promise.all([
      ...
      read("docs/adr/0015-keep-orchestration-independent-from-the-scheduler.md"),
    ...
    expect(scheduler).toContain("Status: Extended by ADR 0019");
