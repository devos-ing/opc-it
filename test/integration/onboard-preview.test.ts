...
it("renders three mode-0600 templates under the contained output", async () => {
  ...
  expect(
    await onboardPreview(validInput, {
      files,
      repositories: repositoryReader({ private: true, fork: false, owner: "devos-ing" }),
    }),
  ).toEqual([
    ".codex-pipeline.yml",
    ".github/ISSUE_TEMPLATE/opc-work.yml",
  ...
  expect(files.writes.get("preview/onboarding/.github/workflows/opc.yml")?.content).toContain(
    `${validInput.controlRepository}/.github/workflows/reusable-opc.yml@${validInput.controlRef}`,
  );
});
...
it.each([
  ...
  [{ ...validInput, approver: "-bad" }, { private: true, fork: false, owner: "devos-ing" }, "INVALID_GITHUB_LOGIN"],
  [{ ...validInput, repository: "invalid" }, { private: true, fork: false, owner: "devos-ing" }, "INVALID_REPOSITORY"],
  [{ ...validInput, controlRepository: "devos-ing" }, { private: true, fork: false, owner: "devos-ing" }, "INVALID_GITHUB_REPOSITORY"],
  [validInput, { private: false, fork: false, owner: "devos-ing" }, "UNTRUSTED_REPOSITORY"],
  [validInput, { private: true, fork: true, owner: "devos-ing" }, "UNTRUSTED_REPOSITORY"],
