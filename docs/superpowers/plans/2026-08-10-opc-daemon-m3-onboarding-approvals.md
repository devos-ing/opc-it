...
**Files:** Create `src/cli/commands/{onboard,submit,status,pause,resume,doctor,uninstall,daemon,output}.ts`, `src/cli/production.ts`, and purpose-specific `src/cli/production/{shared,daemon,inspection,uninstall}.ts`; modify `src/cli/main.ts`, the public `src/features/{queue,approvals}/index.ts` seams, `src/features/approvals/ports.ts`, `src/platform/approvals/telegram-approval-adapter.ts`, and `scripts/build.ts`; test `test/unit/cli-smoke.test.ts`, `test/unit/cli-inspection.test.ts`, `test/acceptance/onboarding-flow.test.ts`.
...
Task 5 evidence: the first focused RED for `onboard --preview` reached no factory and failed because the command was unknown; subsequent command, uninstall-confirmation, and unrelated-factory-accessor REDs also failed before their implementations. The CLI now parses bounded, NUL-free argv before resolving only the selected own data factory, binds every mutation to a freshly loaded exact `sha256:` preview, and emits exactly one stdout/stderr JSON line through command-owned typed closed codecs that require complete known structures and reject proxies, prototype-name fields, unknown structure, and opaque secret strings. Lazy production composition stages explicit GitHub identity and per-repository confirmation before identity grants, then separately applies a disabled current-user install and activation that rechecks the permission manifest plus live GitHub and Codex identities before mutation. The LaunchAgent `daemon --config` entry composes enabled authority, SQLite journal/process lock, delivery loop, health, signal handling, and independently tested two-handle cleanup; `status` and `doctor` expose the planned identity, poll, lease, outbox, repository, sandbox, SQLite, Telegram, and manifest checks. Doctor arbitrates the complete signed repository journal before applying the existing canonical lease/heartbeat/outage rules, so Recovery handoffs and losing claims cannot create false stuck leases and a forged long expiry cannot extend the fixed lease. It also rejects future poll health, combines pending message and transition outboxes, and treats Telegram as paired only when a production-valid token and canonical user/chat pairing all exist. Uninstall defaults to preserving all audit data and credentials and requires digest-bound, independent flags before stopping the job or removing program files, state/logs, Telegram token, or transition key. Focused CLI plus inspection and legacy acceptance passes 38 tests / 220 expectations; the full suite passes 693 tests / 1,832 expectations; lint, typecheck, build, diff checks, nine-command bundle inspection, and secret-literal scans pass. All acceptance work used pure inputs, injected fakes, and temporary files only: no production adapter, network, Keychain, launchctl, GitHub, Codex, Telegram, or host mutation ran. `upgrade` remains intentionally deferred to M5 Task 2. Independent Spec and Standards reviews report 0 findings each.
