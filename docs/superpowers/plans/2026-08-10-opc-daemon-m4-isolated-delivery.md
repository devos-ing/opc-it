...
M4 final whole-milestone closure evidence (2026-08-12): the final review replay first exposed four remaining production/crash boundaries and each was closed with a targeted RED before the final GREEN. Attempt-three exhaustion now preserves its pending root continuation when the child becomes blocked and can idempotently finish the exact already-signed root terminalization after the original execution deadline without reopening work authority. The first signed infrastructure `outage_started_at` survives reclaim, lease heartbeat, and repeated incidents so the continuous 24-hour blocker cannot be reset by a healthy daemon. Evidence bundles create a canonical mode-0600 sibling reservation before the deterministic directory, then require exact path, containment, current uid, mode, content digest/bytes, device/inode, and an opaque in-process token for every adoption or deletion; completed-process and pre-marker crash replays recover safely, while cleanup preserves primary and secondary failures in order. Publisher's real Git credential chain is modeled as Git actually executes it: only the Publisher role under exact `github.com:443` authority receives exact child `process-exec` grants for canonical `/bin/sh`, the approved `gh`, and `git-remote-https`; `/bin/sh` remains rejected as a top-level Publisher request, all helper argv are fixed from a validated absolute `gh` path, and all other credentials and network remain denied. Final focused M4 verification is `119 pass / 1 exact parent-Seatbelt skip / 0 fail` with `420` expectations; `test/acceptance/daemon-delivery-loop.test.ts` passes twice at `8 pass / 0 fail` with `34` expectations each; full verification is `876 pass / 1 exact skip / 0 fail` with `2663` expectations. Lint, typecheck, build, and diff checks exit `0`; independent final whole-M4 Spec and Standards/security re-reviews both report `0 findings`. No real GitHub endpoint, credential helper, credential content, network push, Codex session, Target command, Keychain, LaunchAgent activation, or persistent repository was accessed; publication and failure scenarios used injected boundaries and cleaned temporary repositories only.
