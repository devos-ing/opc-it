...
QA repair evidence (2026-08-12): the local production composition now performs local-only apply through an injectable fixed-path private filesystem adapter rather than rejecting with `UPGRADE_PRODUCTION_ADAPTER_REQUIRED`; the temp-host acceptance proves exact dual artifact replacement and excludes lifecycle/process lock files. The closed output now reports old config plus old/new CLI and wrapper digests, and exact permission before/after differences. Rollback stops/proves candidate, restores snapshot, restarts/verifies previous runtime, then clears the fence; the multi-migration validator uses a non-TDZ monotonic schema check.
